function [Best_score,Best_pos,cg_curve,Food_Product]=DA(SearchAgents_no,Max_iteration,lb,ub,dirt,fobj,samp_i,fixnums)
dim = length(lb); 
...
if size(ub,2)==1
    ub=ones(1,dim)*ub;
    lb=ones(1,dim)*lb;
    dirt=ones(1,dim)*dirt;
end


%初始化蜻蜓邻里半径
Delta_max=5*didrt; %令最大变化为3倍dirt zfy
...
X=initialization(SearchAgents_no,dim,ub,lb,dirt);
Fitness=zeros(1,SearchAgents_no); 

DeltaX=initialization(SearchAgents_no,dim,ub,lb,dirt);
